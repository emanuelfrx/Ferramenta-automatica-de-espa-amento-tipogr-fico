...
export const MetricTuner: React.FC<MetricTunerProps> = ({ settings, onSettingsChange, fontFamily, font }) => {
  ...
  const updateOverride = (side: 'lsb' | 'rsb', val: number) => {
      ...
      // If we are setting a value, we first need to lock in the other side if it was null (rule based)
      // to avoid it resetting to 0 if our logic was strict, but here we can just update the one we touched.
      // However, if the other side is null, we should probably fetch the current rule value and save it 
      // so the user doesn't accidentally zero it out? 
      // Actually, let's just save what we see.
  ...
  const overrideContext = useMemo(() => {
     if (overrideChar === overrideChar.toUpperCase()) {
            ...
            <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                 ...
                 <div className="flex items-center gap-4 mb-6">
                     <select 
                        ...
                        className="bg-gray-900 border border-gray-600 rounded px-3 py-2 text-white font-mono"
                     >
                         {allChars.split('').map(c => <option key={c} value={c}>{c}</option>)}
                     </select>
                     ...
                     {hasOverride && (
                         <button onClick={resetOverride} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1">
